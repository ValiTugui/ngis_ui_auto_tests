...
Feature: MIPORTAL:  User Journey for Order Tracking and GLH Samples.(E2EUI-855,1292)
...
#    When the user sees the search results pagination entry drop-down with default selection of "10"
  ...
  Scenario Outline:NTS-5064:E2EUI-1292,E2EUI-2579: MI Dashboard | GLH Samples
    ...
    And the user sees the selected "<value2>" in the "glh_samples-search-value" value drop-down
    Then the selected search option is reset after test
    ###For 3rd filter-Referral ID
    And the user selects a value "<filter_value3>" from the "glh_samples-search-col" column drop-down
    And the user selects a search operator "<operator1>" from the "glh_samples-search-operator" operator drop-down
    And the user fills in a "<value3>" in the "glh_samples-search-value" search box
    And the user clicks on Add criteria button
    Then the selected search option is reset after test
    ###For 4th filter-Patient NGIS ID
    And the user selects a value "<filter_value4>" from the "glh_samples-search-col" column drop-down
    And the user selects a search operator "<operator1>" from the "glh_samples-search-operator" operator drop-down
    And the user fills in a "<value4>" in the "glh_samples-search-value" search box
...
#    When the user clicks on the Download CSV button to download the CSV file as "file_submissions_filtered".csv in GLH samples page
    ...
    Examples:
      | mi_stage    | filter_value1 | operator  | value1                    | filter_value2   | value2                                              | filter_value3 | value3                    | filter_value4   | operator1  | value4                    | filter_value5                 | value5                                                                                             | paginationValue | ColumnHeader |
      | GLH Samples | GLH           | is        | London North              | Ordering Entity | Barts Health NHS Trust                              | Referral ID   | r20233302920              | Patient NGIS ID | is exactly | p90920230930              | Clinical Indication Test Type | Arthrogryposis - broad panel WGS: Arthrogryposis                                                   | 25              | GMC,NGIS     |
      | GLH Samples | GLH           | is one of | London North,London South | Ordering Entity | 2gether NHS Foundation Trust,Barts Health NHS Trust | Referral ID   | r20233302920,r20963473082 | Patient NGIS ID | is one of  | p90920230930,p84281960894 | Clinical Indication Test Type | Arthrogryposis - broad panel WGS: Arthrogryposis,Cerebral malformations WGS: Cerebral malformation | 50              | GMC,NGIS     |
