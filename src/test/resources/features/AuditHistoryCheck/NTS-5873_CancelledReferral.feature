...
Feature: NTS-5873:Verify Audit history for a cancelled referral.
  ...
  Scenario Outline:Scenario 1: Verify audit history for a cancelled referral(cancer) for using patient Human readable Id..
   ...
   ##Samples
    ...
    Examples:
      | sampleType                | sampleState         | RecordedBy                            | RevokeMessage                                                                                   |
      | Normal or germline sample | Fresh frozen tumour | ClinicianName=John:HospitalNumber=123 | This referral has been cancelled so further changes might not take effect. Start a new referral |


  @NTS-5873 @Z-LOGOUT @Scenario2
