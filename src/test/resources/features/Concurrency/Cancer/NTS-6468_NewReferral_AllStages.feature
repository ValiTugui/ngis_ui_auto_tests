...
Feature: NTS-6468:Cancer_new_referral_all_stages: Submit and verify the changes on each stages done by another user
  ...
  Scenario Outline: Login as User A and B,Access Same Referral, User B updates each stages, User A verify the message by submitting the referral
    ...
    Examples:
      ...
      | Patient details | FirstName=James123:LastName=Peter123:DOB=20-10-1997:Gender=Male:LifeStatus=Alive:Ethnicity=B - White - Irish | Requesting organisation | The Royal Wolverhampton NHS Trust | Test package | Priority=Urgent    | Responsible clinician | FirstName=Karen:LastName=Smith:Department=Edinburgh,UK | Tumours | TumourType=Brain tumour:SIHMDSLabID=VBN89756 | Answer questions about this tumour | FirstPresentation=Recurrence | Samples | SampleType=Liquid tumour sample:SampleState=Fresh frozen tumour:SampleID=SD6756 | Add sample details | SampleCollectionDate=20-05-2020:SampleComments=Sample Collected | Notes | Notes updated by user2 | Patient choice | Proband=Authorised by clinician | Haematological malignancy: solid sample | Solid tumour sample | Tumour fresh fluid | ClinicianName=John:HospitalNumber=123 | First presentation |
