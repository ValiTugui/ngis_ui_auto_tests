...
Feature: Referral Submit Alert  - New Referral Created by One User and Updated by Another User

  @new_referral @Z-LOGOUT
  Scenario Outline: Verify the alert message at the time of submission of a new referral created by one user and updated by another user
    ##Create New Referral
    Given The user is login to the Test Order Service and create a new referral
      | Angiomatoid Fibrous Histiocytoma | CONCURRENT_USER1_NAME | New Referral |NRF1|
    ##Patient Details
...
#    And the user is navigated to a page with title Print sample forms
    ...
    When the user updates the file NRF1 with Mandatory Stages Completed by User1
    And the user waits max 3 minutes for the update Notes Updated by User2 in the file NRF1
    When the user navigates to the "<testPackage>" stage
    And the user selects the "Routine"
    When the user updates the file NRF1 with User1 Navigates to TestPackage and Selected Routine

#    And the user submits the referral
    ##Below steps need to update based on the actual implementation
