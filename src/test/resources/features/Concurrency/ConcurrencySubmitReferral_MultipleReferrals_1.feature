...
Feature: Submit New Referral for RD flow
  ...
  @Newreferral_RD @Z-LOGOUT
    @NTS-6470_1
  Scenario Outline: User A is unable to submit a proband referral where User B has updated the same family member participant in another referral until it has been checked. .
...
#Login as User A
    ...
    Then the user stores the first name & last name values
    And the user stores date of birth and Gender values
...
#    Then the user reads & validate the patient DOB updated
  ...
  Scenario Outline: Update the patient details  in another referral
    ...
    And the user search the family member with first name,last name,gender & DOB values stored in User1 referral
    Then the patient card displays with Born,Gender and NHS No details
    When the user clicks on the patient card
    Then the user is navigated to a page with title Add missing family member details
    When the user clicks on edit patient details
    Then the user is navigated to a page with title Edit patient details
    And And the user fills in the date of birth "<dateOfBirthUpdated>"
