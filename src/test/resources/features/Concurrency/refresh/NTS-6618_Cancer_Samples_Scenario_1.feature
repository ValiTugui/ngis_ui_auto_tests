...
@Refresh
@Refresh_Cancer
Feature: NTS-6618:Cancer_new_referral_Samples_add: Navigate and verify the changes on Samples stage done by another user
  ###FLOW
  ...
  #Here first time verifying the missing details in the samples stage which missed while creating the referral by User1

  @NTS-6618 @NTS-6618_Scenario1 @Z-LOGOUT
  Scenario Outline: Login as User A,Create a New Referral, Complete all stages and do not submit referral,and updated Samples stage, when B accessed same referral then verified data updated by A.
    ...
    Given The user is login to the Test Order Service and create a new referral
      | Fibro-Osseous Tumour of Bone Differential | CONCURRENT_USER1_NAME | New Referral | NTS-6618_Scenario1 |
    ...
    Then the user updates the file NTS-6618_Scenario1 with Mandatory Stages Completed by User1
    #Samples - Updated by User1
    And the user waits max 10 minutes for the update Tumours details Updated by User2 in the file NTS-6618_Scenario1
    ...
    And the user clicks the Save and Continue button
    And the user clicks the Save and Continue button
    And the user updates the file NTS-6618_Scenario1 with Samples details Updated by User1
    Examples:
  ...
  Scenario Outline: Verified Samples stage of new referral updated by another user
    And the user waits max 20 minutes for the update Mandatory Stages Completed by User1 in the file NTS-6618_Scenario1
    Given The user is login to the Test Order Service and access the given referral
      | CONCURRENT_USER2_NAME | New Referral | NTS-6618_Scenario1 |
   #Samples - Verified by User2
    ...
    And the user updates the file NTS-6618_Scenario1 with Tumours details Updated by User2
    And the user waits max 15 minutes for the update Samples details Updated by User1 in the file NTS-6618_Scenario1
    When the user navigates to the "<Samples>" stage
    And the user selects the existing sample on the landing page by clicking on the chevron right arrow icon
    ...
    And the user updates the file NTS-6618_Scenario1 with Samples details validated by User2
