...
Feature: NTS-6617:Cancer_new_referral_Tumours_add: Navigate and verify the changes on Tumours stage done by another user
 ...
 @NTS-6617 @Z-LOGOUT
  Scenario Outline: Login as User A,Create a New Referral, Complete all stages and do not submit referral,and updated Tumours stage, when B accessed same referral then verified data updated by A.
    ...
    And the user clicks the Save and Continue button
    Then the user updates the file NTS-6617_Scenario1 with Mandatory Stages Completed by User1
    #Tumours - Updated by User1
    And the user waits max 10 minutes for the update Responsible clinician details Updated by User2 in the file NTS-6617_Scenario1
    ...
    And the user clicks the Save and Continue button
    And the user clicks the Save and Continue button
    And the user updates the file NTS-6617_Scenario1 with Tumours details Updated by User1
    Examples:
  ...
  Scenario Outline: Verified Tumours stage of new referral updated by another user
    And the user waits max 20 minutes for the update Mandatory Stages Completed by User1 in the file NTS-6617_Scenario1
    Given The user is login to the Test Order Service and access the given referral
      | CONCURRENT_USER2_NAME | New Referral | NTS-6617_Scenario1 |
   #Tumours - Verified by User2
    ...
    And the user updates the file NTS-6617_Scenario1 with Responsible clinician details Updated by User2
    And the user waits max 10 minutes for the update Tumours details Updated by User1 in the file NTS-6617_Scenario1
    When the user navigates to the "<Tumours>" stage
    And the user selects the existing tumour on the landing page by clicking on the chevron right arrow icon
    And the user clicks the Save and Continue button
    Then the "<pageTitle1>" page is displayed
    ...
    And the user updates the file NTS-6617_Scenario1 with Tumours details validated by User2
