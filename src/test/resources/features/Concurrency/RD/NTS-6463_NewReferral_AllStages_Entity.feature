...
Feature: NTS-6463:RD_existing_referral_all_stages_entity
  ...
  Scenario Outline: Login as User A,Create a New Referral, Complete all stages and do not submit referral,and validate the data updated, when B is updating every stage upon referral submission by A.
    ...
    Given The user is login to the Test Order Service and create a new referral
      | Rare syndromic craniosynostosis or isolated multisuture synostosis | CONCURRENT_USER1_NAME | New Referral | NTS-6463 |
    ...
    Then the user updates the file NTS-6463 with Mandatory Stages Completed by User1
    And the user waits max 10 minutes for the update PatientDetails Updated by User2 in the file NTS-6463
    ...
    And the user updates the file NTS-6463 with Patient details validated by User1
   #Requesting Organization - Verify
    And the user waits max 8 minutes for the update Requesting Organisation Updated by User2 in the file NTS-6463
    And the user submits the referral for Concurrency
    Then the user click on Reload referral button to validate the data
    When the user navigates to the "<RequestingOrganisation>" stage
    Then the user verifies the stage "<RequestingOrganisation>" with "<RequestingOrganisationUpdated>"
    And the user updates the file NTS-6463 with Requesting Organisation validated by User1
   #Test Package - Verify
    And the user waits max 12 minutes for the update Test Package details Updated by User2 in the file NTS-6463
    And the user submits the referral for Concurrency
    Then the user click on Reload referral button to validate the data
    When the user navigates to the "<TestPackage>" stage
    And the user verifies the stage "<TestPackage>" with "<TestPackageUpdated>"
    And the user updates the file NTS-6463 with Test Package details validated by User1
   #Responsible Clinician - Verify
    And the user waits max 12 minutes for the update Responsible Clinician details Updated by User2 in the file NTS-6463
    And the user submits the referral for Concurrency
    Then the user click on Reload referral button to validate the data
    When the user navigates to the "<ResponsibleClinician>" stage
    Then the user verifies the stage "<ResponsibleClinician>" with "<ResponsibleClinicianDetailsUpdated>"
    Then the user updates the file NTS-6463 with Responsible Clinician details validated by User1
   #ClinicalQuestions - Verify
    And the user waits max 8 minutes for the update Clinical Questions details Updated by User2 in the file NTS-6463
    And the user submits the referral for Concurrency
    Then the user click on Reload referral button to validate the data
    When the user navigates to the "<ClinicalQuestions>" stage
    Then the user verifies the stage "<ClinicalQuestions>" with "<ClinicalQuestionDetailsUpdated>"
    Then the user updates the file NTS-6463 with Clinical Questions details validated by User1
   #Notes - verify
    And the user waits max 8 minutes for the update Notes details Updated by User2 in the file NTS-6463
    And the user submits the referral for Concurrency
    Then the user click on Reload referral button to validate the data
   ...
   #PatientChoice- Verifyt Details - Verify
    ...
    Then the user updates the file NTS-6463 with Patient Choice details validated by User1
   #Panels-Verify
    And the user waits max 8 minutes for the update Panels details Updated by User2 in the file NTS-6463
    And the user submits the referral for Concurrency
    Then the user click on Reload referral button to validate the data
    When the user navigates to the "<Panels>" stage
    Then the user verifies the stage "<Panels>" with "<PanelsDetailsUpdated>"
    Then the user updates the file NTS-6463 with Panels details validated by User1
  ...
  Scenario Outline: Update entity in every stage of new referral created by another user
    And the user waits max 30 minutes for the update Mandatory Stages Completed by User1 in the file NTS-6463
    Given The user is login to the Test Order Service and access the given referral
      | CONCURRENT_USER2_NAME | New Referral | NTS-6463 |
   ...
   ##Patient Details - Update
    ...
    And the user clicks the Save and Continue button
    Then the user updates the file NTS-6463 with PatientDetails Updated by User2
   ##Requesting Organisation - Update
    And the user waits max 8 minutes for the update Patient details validated by User1 in the file NTS-6463
    When the user navigates to the "<RequestingOrganisation>" stage
    And the user updates the stage "<RequestingOrganisation>" with "<RequestingOrganisationUpdated>"
    And the user clicks the Save and Continue button
    Then the user updates the file NTS-6463 with Requesting Organisation Updated by User2
   ##Test Package - Update
    And the user waits max 8 minutes for the update Requesting Organisation validated by User1 in the file NTS-6463
    When the user navigates to the "<TestPackage>" stage
    And the user updates the stage "<TestPackage>" with "<TestPackageUpdated>"
    And the user clicks the Save and Continue button
    And the user updates the file NTS-6463 with Test Package details Updated by User2
   ##Responsible Clinician- Update
    And the user waits max 10 minutes for the update Test Package details validated by User1 in the file NTS-6463
    When the user navigates to the "<ResponsibleClinician>" stage
    Then the user updates the stage "<ResponsibleClinician>" with "<ResponsibleClinicianDetailsUpdated>"
    And the user clicks the Save and Continue button
    Then the user updates the file NTS-6463 with Responsible Clinician details Updated by User2
   ##ClinicalQuestions - Update
    And the user waits max 8 minutes for the update Responsible Clinician details validated by User1 in the file NTS-6463
    When the user navigates to the "<ClinicalQuestions>" stage
    Then the user updates the stage "<ClinicalQuestions>" with "<ClinicalQuestionDetailsUpdated>"
    And the user clicks the Save and Continue button
    Then the user updates the file NTS-6463 with Clinical Questions details Updated by User2
   ##Notes
    And the user waits max 8 minutes for the update Clinical Questions details validated by User1 in the file NTS-6463
    When the user navigates to the "<Notes>" stage
   ...
   ##PatientChoice- update
    ...
    Then the user updates the file NTS-6463 with Patient Choice details Updated by User2
   ##Panels- update
    And the user waits max 8 minutes for the update Patient Choice details validated by User1 in the file NTS-6463
    When the user navigates to the "<Panels>" stage
    Then the user updates the stage "<Panels>" with "<PanelsDetailsUpdated>"
    And the user clicks the Save and Continue button
    Then the user updates the file NTS-6463 with Panels details Updated by User2
