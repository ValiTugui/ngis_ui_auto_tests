...
Feature: Submit New Referral for RD flow
  ...
  @Newreferral_RD @Z-LOGOUT
    @NTS-6470_2
  Scenario Outline:User B is unable to submit an RD family referral where User A has updated the same proband participant in another referral until it has been checked.
...
#    ##Notes
    ...
    Then the user updates the file NRF1 with Mandatory Stages Completed by User2
   # Referral Submission by User1 after proband details updated by user2 in FamilyMember details in another referral
    ...
    Then the user reads & validate the patient gender updated
  ...
  Scenario Outline: Update the patient details  in another referral
    ...
    When the user waits max 20 minutes for the update Mandatory Stages Completed by User2 in the file NRF1
    Then the user is navigated to a page with title Check your patient's details
    And the user edit the patients gender
    And the user clicks the Save and Continue button
    And the user updates the file NRF1 with PatientDetails Updated by User1
