...
#      | Family members | Not entered | Patient choice       | Form library |
  ...
  Scenario Outline: E2EUI-2039: Verify the relevant Patient choice for an Adult with capacity
    Given a referral is created with the below details for the given existing patient record type and associated tests in Test Order System online service
      | TEST_DIRECTORY_PRIVATE_URL | test-selection/clinical-tests | Holoprosencephaly - NOT chromosomal | NGIS | Rare-Disease | NHSNumber=9449306052:DOB=28-04-2007 |
    When the user navigates to the "<Patient choice stage>" stage
    Then the user is navigated to a page with title Patient choice
    When the user edits the patient choice status
    Then the user is navigated to a page with title Add patient choice information
...
#  @COMP8_TO_PatientSearch
#    @patientChoice_Page04 @E2EUI-2039 @v_1 @P0 @scenario_1
#  Scenario Outline: E2EUI-2039: scenario 1 - Verify the relevant Patient choice for an Adult with capacity
...
#    And the user clicks on Continue Button
#    Then the option is marked as completed for option Patient choices
...
#
#  @COMP8_TO_PatientSearch
#    @patientChoice_Page06 @E2EUI-2039 @v_1 @P0 @scenario_3
#  Scenario Outline: E2EUI-2039: scenario 3 - Verify the relevant Patient choice for an Adult with capacity
...
#    And the user selects "<YesOption>" research participation in patient choices option
#    And the user selects "<NoOption>" NHS care in patient choices option
##    And the user selects "<Options>" options in patient choices option
#    Then the user sees a "<WarningMessage1>" warning message on the patient choice information option
#    And the user selects "<NoOption>" research participation in patient choices option
...
#    Then the user sees a "<WarningMessage2>" warning message on the patient choice information option
#    And the user clicks on Continue Button
#    Then the user should see a error message box
#
#    Examples:
#      | PatientChoice1                 | YesOption | NoOption | Question1                                  | WarningMessage1                                                                                                                                                     | Question2                                          | WarningMessage2                                                                                                        |
#      | Patient has agreed to the test | Yes       | No       | Has research participation been discussed? | You have selected \"No\" to participation in research. Please ensure the patient is aware they might be contacted in the future about other research opportunities. | Why has research participation not been discussed? | All patients who receive genomic tests should be offered the opportunity to participate in research where appropriate. |
