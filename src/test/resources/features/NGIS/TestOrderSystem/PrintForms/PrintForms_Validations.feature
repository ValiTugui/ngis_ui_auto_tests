...
Feature: Print Forms - Validations
  ...
  @NTS- @E2EUI-1262 @E2EUI-826 @LOGOUT
  Scenario Outline: NTS-: Download forms from the Print forms stage and verify lab address
    Given a new patient referral is created with associated tests in Test Order System online service
      | TEST_DIRECTORY_PRIVATE_URL | test-selection/clinical-tests | R27 | GEL_NORMAL_USER | NHSNumber=NA-Patient is a foreign national:DOB=25-10-2005:Gender=Male |
    ###Patient Details
    When the user is navigated to a page with title Check your patient's details
    And the user clicks the Save and Continue button
    And the "<PatientDetails>" stage is marked as Completed
    ###Requesting Organisation
    Then the user is navigated to a page with title Add a requesting organisation
    And the user enters the keyword "UNIVERSITY DENTAL HOSPITAL OF MANCHESTER" in the search field
    And the user selects a random entity from the suggestions list
    Then the details of the new organisation are displayed
    And the user clicks the Save and Continue button
    And the "<RequestingOrganisation>" stage is marked as Completed
    ...
    And the user is able to download print form for the proband
    Then the user clicks "Show address" link to verify the address of the lab in the downloaded file "SampleForm.pdf"
    When the user is able to download print forms for "<NoOfParticipants>" family members with the below details
      | FamilyMemberDetails         |
      | NHSNumber=NA:DOB=10-11-1949 |
    Then the user clicks "Show address" link to verify the address of the lab in the downloaded file "SampleForm.pdf"

    Examples:
      | PatientDetails  | RequestingOrganisation  | TestPackage  | FamilyMembers  | PrintForms  | NoOfParticipants |
      | Patient details | Requesting organisation | Test package | Family members | Print forms | 2                |
