...
Feature: Print Forms 1 - Offline Orders
  ...
  @NTS-5982
  Scenario Outline: scenario_2: Design: Interactive PDF forms-Cancer
    Given a web browser is at the Private Test Selection homepage
      | TEST_DIRECTORY_PRIVATE_URL | test-selection/clinical-tests |
    And the user types in the CI term  in the search field and selects the first result from the results list
      | M88 |
    ...
    Examples:
      | Text                                                                                                                                                                                          | RequestingOrganisation                                  |
      | CANCER,Patient conversation taken place; Record of Discussion form to follow,Additional clinical information (if required),Additional tumour information (if relevant),Responsible consultant | Liverpool Heart and Chest Hospital NHS Foundation Trust |
