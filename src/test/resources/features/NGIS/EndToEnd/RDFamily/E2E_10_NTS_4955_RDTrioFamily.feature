...
Feature: RDFamily:NTS-4955: Create Referral for Trio Family with Patient Choice option as No and check DDF payload
  ...
  Scenario Outline: NTS-4955: E2EUI-2667 E2E #10: Create Referral for Trio Family + Default Data + Add Family Members to Test + Patient Choice No
    ...
    And the user enters the keyword "Medway NHS Foundation Trust" in the search field
    ...
    Examples:
      | PatientDetails  | RequestingOrganisation  | TestPackage  | ThreeParticipant | ResponsibleClinician  | ResponsibleClinicianDetails                               | RecordedBy                            | ClinicalQuestion   | ClinicalQuestionDetails                                        | Notes | FamilyMembers  | PatientChoice  | Panels | Pedigree |
      | Patient details | Requesting organisation | Test package | 3                | Responsible clinician | FirstName=Karen:LastName=Smith:Department=Victoria Street | ClinicianName=John:HospitalNumber=123 | Clinical questions | DiseaseStatus=Affected:AgeOfOnset=10,02:HpoPhenoType=Epistaxis | Notes | Family members | Patient choice | Panels | Pedigree |
